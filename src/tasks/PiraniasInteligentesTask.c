...
void task() {
    /*
     * Pirañas inteligentes: son pequeñas pero atacan juntas; están siempre
     * buscando comida de forma inteligente.
     *
     * Primero buscan comida alcanzable por un solo movimiento. Si no la
     * encuentran, buscan comida alcanzable por dos movimientos. Si nuevamente
     * no encuentran, van recorriendo hacia la derecha y abajo.
     *
    ...
    // search_fruit_two_moves_away:

    for (offset_x = -4; offset_x <= 4; ++offset_x) {
        if (offset_x == 0) {
            ++offset_x;
        }
        for (offset_y = -4; offset_y <= 4; ++offset_y) {
            if (offset_y == 0) {
                ++offset_y;
            }
            ...
            if (position_visited[next_x][next_y] == 0) {
                if (syscall_read(offset_x, offset_y) == Food) {
                    if (offset_y > 0) {
                        syscall_move(offset_y, Down);
                    } else {
                        syscall_move(-1*offset_y, Up);
                    }
                    y = next_y;
